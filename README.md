...
This project implements Kruskal’s Algorithm to compute the Minimum Spanning Tree (MST) for a weighted, undirected graph. The program reads input from a file, then processes the graph edges using a greedy approach with a Disjoint Set Union (DSU) structure, and outputs the total weight of the MST along with the selected edges. 
